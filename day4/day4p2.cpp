...
int getCopies(string card) {
    int i = 0;
    string n = "";
    set<int> winningNums;
    vector<int> nums;

    while (i < card.length() && card[i] != '|') {  // parse winning numbers
        if (card[i] == ' ') {
            if (n.length()) {
                winningNums.insert(stoi(n));
                n = "";
            }
        } else {
            n += card[i];
    ...
    while (i < card.length()) {  // parse card nums
        if (card[i] == ' ') {
            if (n.length()) {
    ...
    nums.push_back(stoi(n));  // get last num

    int numCopies = 0;
    for (int n : nums) {
        if (winningNums.count(n)) numCopies++;
    }
...
int main() {
    ...
    string nextCard;
    vector<string> cards;
    while (getline(file, nextCard)) {
        vector<string> gameAndCard = split(nextCard, ':');
        cards.push_back(gameAndCard[1]);
    }
    file.close();

    int res = 0;
    vector<int> numCards(cards.size(), 1);
    for (int i = 0; i < cards.size(); i++) {
        int numCopiesWon = getCopies(cards[i]);
        for (int c = i + 1; c < i + 1 + numCopiesWon; c++) {
            numCards[c] += numCards[i];
        }
