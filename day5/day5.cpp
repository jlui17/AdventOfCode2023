...
vector<long> getSeeds(vector<string> seeds) {
    vector<long> res;
    vector<string> first = split(seeds[0], ' ');
    for (int i = 1; i < first.size(); i++) {
        res.push_back(stol(first[i]));
    }
...
long convertSeed(long seed, vector<tuple<long, long, long>> map) {
    for (auto t : map) {
        long source = get<1>(t);
        long length = get<2>(t);
        if (seed >= source && seed < source + length) {
            long offset = seed - source;
            long dest = get<0>(t);
            return dest + offset;
        }
    }
...
int main() {
    ...
    vector<long> seeds = getSeeds(seedsInput);
    ...
    long minLocation = numeric_limits<long>::max();
    for (long s : seeds) {
        long n = s;
        for (vector<tuple<long, long, long>> map :
             allMaps) {  // convert s -> location
            n = convertSeed(n, map);
        }

        minLocation = min(minLocation, n);
    }
