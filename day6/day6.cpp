...
vector<int> getNums(string input) {
    vector<int> res;
    string n = "";
    for (char c : input) {
        if (c == ' ' && n != "") {
...
int calcNumWaysToWin(int time, int distance) {
    int res = 0;

    for (int speed = 1; speed < time; speed++) {
        int timeRemaining = time - speed;
...
int main() {
    ...
    vector<int> times = getNums(timeInput);
    vector<int> distances = getNums(distanceInput);

    int res = 1;
    for (int i = 0; i < times.size(); i++) {
        res *= calcNumWaysToWin(times[i], distances[i]);
    }
